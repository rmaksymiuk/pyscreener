...
from .dock3 import DOCK3VirtualScreen
...
def build_metadata(software: str, metadata: Optional[Dict] = None) -> SimulationMetadata:
    ...
    if software.lower() in ("dock3.8", "dock3"):
        from pyscreener.docking.dock3.metadata import DOCK3Metadata

        d_md = asdict(DOCK3Metadata())
        d_md.update((k, metadata[k]) for k in d_md.keys() & metadata.keys())

        return DOCK3Metadata(**d_md)
...
def get_runner(software: str) -> DockingRunner:
    ...
    if software.lower() in ("dock3.8", "dock3"):
        from pyscreener.docking.dock3 import DOCK3Runner
